...
const HeroSection = () => (
  <section className="relative overflow-hidden py-24 md:py-32">
    ...
    <div className="pointer-events-none absolute inset-0">
      ...
      <div className="orb-delay absolute right-1/4 bottom-1/4 h-80 w-80 rounded-full bg-primary/8 blur-[120px]" />
      <div className="orb absolute left-1/2 top-1/2 h-48 w-48 -translate-x-1/2 -translate-y-1/2 rounded-full bg-secondary/5 blur-[80px]" />
    </div>

    <div className="container relative text-center">
      <h1 className="font-heading text-4xl font-bold leading-tight md:text-6xl lg:text-7xl">
        The Internet's Best Guide to{" "}
        <span className="gold-gradient-text">Twink Content</span>
      </h1>
      ...
      <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
        <Link
          to="/top-sites"
          className="gold-gradient inline-flex items-center gap-2 rounded-button px-8 py-3.5 text-sm font-semibold text-secondary-foreground transition-opacity hover:opacity-90"
        >
    ...
    <AnimateOnScroll>
      <section className="py-16">
        <div className="container">
          <h2 className="font-heading text-3xl font-bold md:text-4xl">🏆 Editor's Top Picks</h2>
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {featured.map((site) => (
              <Link
                key={site.id}
                to={`/reviews/${site.slug}`}
                className="card-glow flex flex-col rounded-lg border border-card-border bg-card p-5"
              >
                {site.badge && (
                  <span className="mb-3 inline-block self-start rounded-button bg-primary/15 px-2.5 py-1 text-xs font-semibold text-primary">
                    {site.badge}
                  </span>
                )}
                ...
                <span className="mt-4 inline-flex items-center justify-center rounded-button gold-gradient px-4 py-2 text-xs font-semibold text-secondary-foreground">
                  Visit Site
                </span>
              </Link>
            ))}
...
const CategoriesSection = () => (
  <AnimateOnScroll>
    <section className="py-16">
      <div className="container">
        <h2 className="font-heading text-3xl font-bold md:text-4xl">Browse By Category</h2>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((cat) => (
            <Link
              key={cat.slug}
              to={`/category/${cat.slug}`}
              className="card-glow flex items-center gap-4 rounded-lg border border-card-border bg-card p-5 transition-colors"
            >
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {latest.map((site) => (
              <Link
                key={site.id}
                to={`/reviews/${site.slug}`}
                className="card-glow rounded-lg border border-card-border bg-card p-6"
              >
...
const TrustSection = () => (
  <AnimateOnScroll>
    <section className="border-t border-border py-16">
      <div className="container">
        <h2 className="text-center font-heading text-3xl font-bold md:text-4xl">Why Trust Us</h2>
